                    ...
                    <table class="table table-striped table-bordered table-hover" id="table_user">
                        ...
                        <tbody>
                            ...
                            <tr class="odd gradeX">
                                ...
                                <td class="name" style="width:8px;"><?php echo $value["name"]; ?></td>
                                <td class="descr" value=""><?php echo $value["description"]; ?></td>
...
<script>
    ...
    jQuery(document).on('click', '.icon-edit', function() { //alert('xxxxxxxx');
        ...
        //var name = $(this).parents().parents().children([name='name']).text();
        //alert(name);
        //var descr = $(this).parents().parents().children([name='descr']).text();
        var name = $(this).parents().prev().prev().text();
        var descr = $(this).parents().prev().text();
        
        var text = '<form action="<?php echo base_url() ?>dashboard/page_edit" method="post" id="form_user_edit" class="">';
        text += '<div class="controls">'
        text += '<label class="control-label">Page</label>';
        text += '<input type="text" name="username" value="'+name+'" class="" >'
        text += '<label class="control-label">Mô tả</label>';
        text += '<input type="text" name="email" value="'+descr+'" class="" >'
                    ...
                    action: function() {
                        ...
                        $.ajax({
                            ...
                            success: function(data) { 
                                if (data != false)
                                {
                                    alert('bạn đã update page thành công');
                                    BootstrapDialog.closeAll();

                                }
                                else {
                                    alert('bạn đã update page ko thành công');
                                }
                            },
                        });
                    }
    ...
    jQuery(document).on('click', '#create_user', function() { 
        var text = '<form action="<?php echo base_url() ?>dashboard/create_page" method="post" id="form_create_user" class="">';
        var that = $(this).parent().parent();
        text += '<div class="controls">'
        text += '<label class="control-label">Tên trang</label>';
        text += '<input type="text" name="name" class="m-wrap large" placeholder="Tên trang">'
        text += '<label class="control-label">Mô tả</label>';
        text += '<input type="text" name="descr" class="m-wrap large" placeholder="Mô tả">'
        text += '</div>'
        text += '</form>'
        BootstrapDialog.show({
            title: 'Tạo trang mới',
                        ...
                        $.ajax({
                            ...
                            success: function(data) { 
                                if (data.stt == true)
                                {
                                    ...
                                    BootstrapDialog.closeAll();
                                    alert('bạn đã tạo trang mới thành công');
                                }
                                else {
                                    alert('bạn chưa tạo thành công');
                                }
                            },
                        });
                    }
